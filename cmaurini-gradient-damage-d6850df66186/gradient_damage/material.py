...
class GradientDamageMaterial(object):
	...
	self.kappa=self.E/(2*(1.0 + self.nu)*(1. - 2.0 * self.nu))
    ...
    def pressurized_energy_density (self, u, alpha):
      	"""
      	 Pressurized energy density
       	"""
	return -(self.C_biot-1.) * ( pow((1.-alpha),2) * self.Pressure(u) * div(u) )
       	#return -(self.C_biot-1.) * ( pow((1.-alpha),2) * self.Pressure * div(u) ) + dot( pow((1.-alpha),2) * grad(self.Pressure), u)  
