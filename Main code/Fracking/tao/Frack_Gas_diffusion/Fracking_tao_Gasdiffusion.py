...
def Fracking(E, nu, hsize, ell, law, ModelB, ka, kb, q, pressure_steps):
	...
	kappa= 1.e-12 #is the permeability of the rock mm^2
	mu_dynamic= 4.04e-11  #is the dynamic viscosity of the fluid MPa
	...
	# others definitions
	#=======================================================================================
	prefix = "%s-L%s-H%.2f-S%.4f-l%.4f-ka%s-kb%s-steps%s"%(law,L,H,hsize, ell, ka, kb, pressure_steps)
	...
	class Void(SubDomain):
	    def inside(self, x, on_boundary):
		return   x[0] <= L/2+2*radius and x[0] >= L/2-2*radius  and  x[1] <= H/2+2*radius and x[1] >= H/2-2*radius  and on_boundary
	...
	#====================================================================================
	#Pressure = div(u_)*P*dx- div(u_0)*P*dx+phi/N*P_*P*dx + DeltaT*(kappa*exp(5*alpha_)/mu_dynamic)*rho*inner(nabla_grad(P_), nabla_grad(P))*dx-(phi/N*P_0 +DeltaT*f)*P*dx-rho*DeltaT*Q*P*ds(5)
	Pressure = div(u_)*P*dx- div(u_0)*P*dx+phi/N*P_*P*dx + DeltaT*(kappa*exp((ka*alpha_)**kb)/mu_dynamic)*rho*inner(nabla_grad(P_), nabla_grad(P))*dx-(phi/N*P_0 +DeltaT*f)*P*dx-DeltaT*Q*P*ds(5)
	...
	for (i_p, p) in enumerate(load_multipliers):
	    ...
	    Borehole_Pressure[i_p] = np.array([p, P_point_a, P_point_b, P_point_c,  P_point_d])
	...
	#def plot_PressureEvolution():
	   ...
	   # p1, = plt.plot(Borehole_Pressure[:,0], Borehole_Pressure[:,1],'b-o',linewidth=2)
	   # p2, = plt.plot(Borehole_Pressure[:,0], Borehole_Pressure[:,2],'r-o',linewidth=2)
	   # p3, = plt.plot(Borehole_Pressure[:,0], Borehole_Pressure[:,3],'k-o',linewidth=2)
	   # p4, = plt.plot(Borehole_Pressure[:,0], Borehole_Pressure[:,4],'g-o',linewidth=2)
	   # plt.legend([p1, p2, p3, p4], ["Point A","Point B","Point C", "Point D"])
	...
	forces_r = assemble(inner(sigma(u_,alpha_)*e1,e1)*ds(1))
	forces_l = assemble(inner(sigma(u_,alpha_)*e1,e1)*ds(2))
	forces_t = assemble(inner(sigma(u_,alpha_)*e1,e1)*ds(3))
	forces_b = assemble(inner(sigma(u_,alpha_)*e1,e1)*ds(4))

	print "forces_r", forces_r
	print "forces_l", forces_l
	print "forces_t", forces_t
	print "forces_b", forces_b

	VV = VectorFunctionSpace(mesh, "CG", 1)
	Velocity_darcy = -kappa*exp( (ka*alpha_)**kb )*Identity(ndim) /mu_dynamic *nabla_grad(P_)
