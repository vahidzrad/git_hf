...
def Fracking(E, nu, hsize, ell, law, ModelB, ka, kb, q, pressure_steps):
	...
	sigma_R = Expression(("-p", "0.",), p=0.0, degree=1) #traction on TOP boundary
	sigma_T = Expression(("0.", "-p",), p=0.0, degree=1) #traction on TOP boundary

	#====================================================================================
	# Define  problem and solvers
	#====================================================================================
	Pressure = 1./M_biot*P_*P*dx +DeltaT*kappa*exp( pow(ka*alpha_, kb) )/mu_dynamic*inner(nabla_grad(P), nabla_grad(P_))*dx-(1/M_biot*P_0 +DeltaT*f)*P*dx-DeltaT*Q*P*ds(5) # Wang et. al 2017 eq(8)
	#------------------------------------------------------------------------------------
	...
	for (i_p, p) in enumerate(load_multipliers):
	    ...
	    print"\033[1;32m--- Time step %d: time = %g, Flowrate= %g ---\033[1;m" % (i_p, i_p*DeltaT, p*q)
	    #u_R.p = p*ut
	    #u_L.p = p#*ut
	    #u_B.p = p#*ut
	    #u_T.p = p*ut
